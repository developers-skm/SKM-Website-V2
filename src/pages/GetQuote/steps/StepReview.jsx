...
function ReviewRow({ label, value }) {
  ...
  return (
    <div className="flex items-start justify-between gap-4 py-2.5 border-b border-surface-100 dark:border-surface-800/60 last:border-b-0">
      <span className="font-body text-[12px] font-semibold uppercase tracking-wide text-surface-400 dark:text-surface-500 flex-shrink-0">{label}</span>
      <span className="font-body text-[13.5px] text-surface-800 dark:text-white text-right">{value}</span>
    </div>
  );
}
...
export default function StepReview({ formData }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="rounded-[14px] border border-[#eee] dark:border-surface-800 bg-white dark:bg-surface-900/40 px-5 sm:px-6 py-2">
        {isSampleCart ? (
