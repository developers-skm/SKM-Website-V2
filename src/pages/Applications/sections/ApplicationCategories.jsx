            ...
            return (
              ...
              >
                ...
                <div className="relative flex flex-col gap-2 px-7 pt-6 pb-7">
                  ...
                  <h3 className="font-heading font-bold text-[22px] text-heading leading-[1.2] m-0 transition-transform duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:-translate-y-0.5">
                    {app.title}
                  </h3>
                  <p className="font-body text-[15px] text-surface-600 leading-[1.55] m-0">
                    {app.problem}
                  </p>
                  {matchedProducts.length > 0 && (
                    <p className="font-body text-[13px] text-surface-400 mt-1 m-0">
                      {matchedProducts.length} matching {matchedProducts.length === 1 ? 'product' : 'products'}
                    </p>
                  )}
                  <button
                    onClick={() => onPageChange(app.page)}
                    className="mt-3 self-start inline-flex items-center gap-1.5 font-body font-bold text-[12.5px] uppercase tracking-[0.05em] text-brand-600 hover:text-[#a80000] cursor-pointer bg-transparent border-none p-0 active:scale-[0.98] transition-transform duration-200 ease-[cubic-bezier(0.22,1,0.36,1)]"
                  >
