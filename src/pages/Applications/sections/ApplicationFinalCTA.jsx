...
export default function ApplicationFinalCTA({ application, onPageChange }) {
  ...
  return (
    <section className="relative w-full overflow-hidden bg-[#fdf1f0] dark:bg-brand-950/20 py-14 sm:py-20 lg:py-24">
      <motion.div
        className="absolute left-1/2 top-1/2 w-[420px] h-[280px] rounded-[50%] border border-brand-500/15 dark:border-brand-400/10 pointer-events-none"
        style={{ x: '-50%', y: '-50%' }}
        initial={reduceMotion ? { opacity: 0.5, scale: 1 } : { opacity: 0, scale: 0.85 }}
      ...
      />
      <motion.div
        className="mx-auto max-w-[900px] w-full px-5 sm:px-8 lg:px-12 flex flex-col items-center text-center gap-5"
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true, margin: '-80px' }}
        variants={{ hidden: {}, visible: { transition: { staggerChildren: reduceMotion ? 0 : STAGGER } } }}
      >
        <motion.span variants={itemVariants} transition={{ duration: reduceMotion ? 0.01 : DURATION.sectionEntrance, ease: EASE_PREMIUM }} className="inline-flex items-center gap-2 font-body text-[12px] font-semibold uppercase tracking-[0.14em] text-brand-600 dark:text-brand-400">
          <span className="w-5 h-px bg-brand-500" aria-hidden="true" />
          {application.title}
        </motion.span>
        <motion.h2 variants={itemVariants} transition={{ duration: reduceMotion ? 0.01 : DURATION.sectionEntrance, ease: EASE_PREMIUM }} className="font-heading font-bold text-[28px] sm:text-[36px] text-heading dark:text-white m-0 tracking-tight leading-[1.15] max-w-[24ch]">
        ...
        <motion.div variants={itemVariants} transition={{ duration: reduceMotion ? 0.01 : DURATION.sectionEntrance, ease: EASE_PREMIUM }} className="flex flex-wrap items-center justify-center gap-x-6 gap-y-3 mt-2">
          <button
            ...
            className="inline-flex items-center justify-center gap-2.5 bg-brand-600 hover:bg-[#a80000] text-white font-heading font-bold text-[13px] uppercase tracking-[0.05em] leading-none px-8 py-[17px] rounded-[200px] transition-all duration-200 shadow-[0_8px_24px_rgba(228,10,24,0.22)] hover:shadow-[0_10px_30px_rgba(228,10,24,0.32)] hover:-translate-y-0.5 active:translate-y-0 cursor-pointer"
          >
