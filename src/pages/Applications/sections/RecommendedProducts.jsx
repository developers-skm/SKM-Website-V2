        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {matchedProducts.map((product) => {
            ...
            return (
              <div
                key={product.id}
                className="flex flex-col rounded-[24px] overflow-hidden border border-surface-200/60 dark:border-surface-800 bg-white dark:bg-surface-900/40 shadow-[0_3px_14px_rgba(36,30,24,0.05)]"
              >
                ...
                <div className="flex flex-col gap-4 px-6 py-6">
                  ...
                  <div className="flex flex-wrap items-center gap-x-5 gap-y-2 pt-3 mt-1 border-t border-surface-200/60 dark:border-surface-800">
                    <button
                      ...
                      className="inline-flex items-center min-h-[40px] font-body font-bold text-[12px] uppercase tracking-[0.05em] text-heading dark:text-white hover:text-brand-600 dark:hover:text-brand-400 cursor-pointer bg-transparent border border-surface-200 dark:border-surface-700 rounded-full px-4"
                    >
                      View Product
                    </button>
                    <button
                      onClick={() => onPageChange('get-quote', { productId: product.id, applicationId: application.id })}
                      className="inline-flex items-center min-h-[40px] font-body font-bold text-[12px] uppercase tracking-[0.05em] text-white bg-brand-600 hover:bg-[#a80000] cursor-pointer rounded-full px-4"
                    >
                      Request Sample
                    </button>
                    <button
                      ...
                      className={`inline-flex items-center gap-1.5 min-h-[40px] font-body font-bold text-[12px] uppercase tracking-[0.05em] cursor-pointer bg-transparent border-none px-1 ${
