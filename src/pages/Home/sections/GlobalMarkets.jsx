...
function CountBig({ value }) {
  ...
  return (
    <strong ref={ref} className="block font-heading font-black text-brand-600 leading-[0.85] tracking-tight tabular-nums text-[26vw] sm:text-[110px] lg:text-[132px] m-0">
      {display}
    </strong>
  );
}
      ...
      <div className="mx-auto max-w-[1680px] w-full px-6 sm:px-10 lg:px-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 items-center mb-14 lg:mb-16">
          ...
          <motion.div {...fadeUp(reduceMotion)} className="lg:col-span-4 flex flex-col gap-5 lg:relative lg:z-10">
            ...
            <h2 className="font-heading font-bold text-[36px] sm:text-[44px] lg:text-[48px] text-heading leading-[1.05] tracking-tight m-0">
              Exporting to
            </h2>
            <CountBig value="30+" />
            <p className="font-heading font-bold text-[24px] sm:text-[30px] lg:text-[34px] text-heading leading-[1.1] tracking-tight -mt-4 m-0">
              countries.
            </p>
            <p className="font-body text-[15px] lg:text-[16px] text-surface-600 leading-[1.7] max-w-xs mt-2 m-0">
        ...
        <motion.div {...fadeUp(reduceMotion, { delay: 0.1 })} className="border-t border-surface-200/70 pt-10 lg:pt-12">
          <div className="flex flex-wrap items-baseline justify-between gap-4 mb-9">
            <span className="font-body text-[13px] font-semibold uppercase tracking-wide text-surface-400">
              Our Markets
            </span>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-x-10 gap-y-10">
            {marketsByRegion.map((group, i) => (
              ...
              >
                <h4 className="font-heading font-bold text-[13px] uppercase tracking-wide text-brand-600 m-0">
                  {group.region}
                </h4>
                <p className="font-body text-[15px] lg:text-[16px] text-surface-600 leading-[1.75] m-0">
        ...
        <motion.div {...fadeUp(reduceMotion, { delay: 0.15 })} className="grid grid-cols-1 sm:grid-cols-3 gap-6 mt-12 lg:mt-14 pt-10 lg:pt-12 border-t border-surface-200/70">
          {branches.map((branch) => (
            ...
            >
              <span className="font-heading font-bold text-[16px] text-heading group-hover:text-brand-600 transition-colors duration-[250ms]">
                {branch.name}
              </span>
              <span className="font-body text-[13.5px] text-surface-500 leading-[1.5]">
                {branch.tagline}
              </span>
            </InternalLink>
          ))}
