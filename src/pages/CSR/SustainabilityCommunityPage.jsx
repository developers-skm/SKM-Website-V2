...
function EnvironmentalResponsibilitySection({ onPageChange }) {
  ...
  return (
    <div id="sustainability-priorities" className="w-full py-[60px] lg:py-[85px] border-b border-[#eee] bg-white scroll-mt-[100px] xl:scroll-mt-[120px]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {ENVIRONMENTAL_INITIATIVES.map((item, i) => (
            ...
            >
              ...
              <p className="font-body text-[13px] text-surface-500 leading-[1.5] m-0">{item.fact}</p>
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-5">
          {PERFORMANCE_INDICATORS.map((stat, i) => (
            <motion.div
              ...
              className="p-5 bg-brand-600/4 border border-[rgba(228, 10, 24,0.10)] rounded-[10px] flex flex-col gap-1.5"
            >
              <span className="font-heading font-bold text-[24px] sm:text-[26px] text-brand-600 leading-tight">{stat.value}</span>
              <span className="font-body text-[12px] font-medium text-surface-700 leading-tight">{stat.label}</span>
              <span className="font-body text-[10.5px] text-surface-400 uppercase tracking-wide">{stat.source}</span>
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {GOVERNANCE_COMMITMENTS.map((point, i) => (
            <motion.div
              ...
              viewport={{ once: true, margin: '-60px' }}
              transition={{ duration: reduceMotion ? 0.01 : 0.4, delay: reduceMotion ? 0 : i * 0.05 }}
              className="flex items-start gap-4 p-4 bg-white border border-[#eee] rounded-[10px]"
            >
              <span className="flex-shrink-0 w-8 h-8 rounded-[6px] bg-brand-600/6 border border-brand-600/12 flex items-center justify-center text-brand-600">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                </svg>
              </span>
              <p className="font-body text-[14px] text-surface-600 leading-relaxed m-0">{point}</p>
            </motion.div>
          ))}
        </div>
      ...
      <div className="w-full flex flex-col bg-page">
        ...
        <div className="w-full pt-[110px] pb-[70px] sm:pt-[130px] lg:pt-[100px] lg:pb-[100px] border-b border-[#eee] text-center px-4">
          ...
          >
            ...
            <p className="font-body text-[15px] sm:text-[16px] text-surface-500 leading-[28px] m-0 max-w-2xl">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 mt-2">
              ...
              <button
                ...
                title="Coming soon"
                aria-disabled="true"
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2.5 min-h-[46px] px-7 py-3 rounded-full border border-surface-300 text-surface-400 font-heading font-bold text-[13px] uppercase tracking-[0.04em] cursor-not-allowed opacity-60"
              >
