  ...
  return (
    <span
      className={`inline-flex items-center font-body text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full ${
        isFlyer
          ? 'text-brand-600 bg-brand-600/8 border border-brand-600/15'
...
function BrochureCard({ entry, large }) {
  return (
    <motion.div
      ...
      className={`group relative flex flex-col gap-5 p-6 sm:p-8 rounded-[20px] border border-surface-200/70 bg-white shadow-[0_4px_20px_rgba(36,30,24,0.05)] hover:shadow-[0_18px_44px_rgba(36,30,24,0.12)] hover:border-brand-600/25 hover:-translate-y-1.5 transition-all duration-300 overflow-hidden ${large ? 'sm:col-span-1' : ''}`}
    >
      {/* Accent corner glow */}
      <div
        className="absolute -top-16 -right-16 w-32 h-32 rounded-full bg-brand-600/[0.06] opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
        aria-hidden="true"
      />

      {/* Icon + badge row */}
      <div className="relative flex items-start justify-between gap-3">
        <div className="w-12 h-12 rounded-[14px] bg-gradient-to-br from-brand-600/10 to-brand-600/[0.03] border border-brand-600/15 flex items-center justify-center flex-shrink-0 shadow-[0_2px_8px_rgba(228,10,24,0.08)] group-hover:scale-105 transition-transform duration-300">
      ...
      <div className="relative flex flex-col gap-2 flex-grow">
        <h3 className="font-heading font-bold text-[18px] sm:text-[20px] text-heading leading-snug m-0">
          {entry.title}
        </h3>
        <p className="font-body text-[14px] text-surface-500 leading-[22px] m-0">
          {entry.desc}
        </p>
      </div>

      {/* Actions */}
      <div className="relative flex items-center gap-3 mt-auto pt-5 border-t border-surface-200/60">
        <a
          href={entry.url}
          target="_blank"
        ...
        <a
          href={entry.url}
          download
          className="inline-flex items-center gap-2 text-surface-500 hover:text-brand-600 font-heading font-bold text-[11px] uppercase tracking-[0.05em] px-4 py-2.5 rounded-[200px] border border-surface-200 hover:border-brand-600/30 hover:bg-brand-600/[0.03] transition-all duration-300"
        >
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
      ...
      <div className="w-full bg-page pt-[110px] pb-[90px] sm:pt-[130px] lg:pt-[120px] lg:pb-[120px] overflow-hidden">
        ...
        >
          ...
          <div className="text-center flex flex-col items-center gap-5">
            <motion.span variants={itemVariants} className="inline-flex items-center gap-2 font-body text-[12px] font-semibold uppercase tracking-[0.14em] text-brand-600">
          ...
          {featured.length > 0 && (
            <div className="flex flex-col gap-6">
              <motion.div variants={itemVariants} className="flex items-center gap-4">
                ...
                <h3 className="font-heading font-bold text-[13px] uppercase tracking-widest text-surface-500 m-0">
                  Corporate Documents
                </h3>
