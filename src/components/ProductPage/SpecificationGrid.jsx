    ...
    <table className="w-full border-collapse">
      ...
      <tbody className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-6">
        {entries.map(([key, value]) => (
          <tr key={key} className="flex flex-col gap-1.5 border-t border-surface-200/70 dark:border-surface-800 pt-4">
            <th scope="row" className="text-left font-body text-[11.5px] font-semibold uppercase tracking-wider text-surface-400 dark:text-surface-500 m-0 p-0">
              {key}
            </th>
            <td className="font-heading font-bold text-[18px] text-heading dark:text-white leading-snug p-0">
              {value}
            </td>
          </tr>
        ))}
