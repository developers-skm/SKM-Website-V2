...
export default function ProductHero({
  ...
  const itemTransition = { duration: reduceMotion ? 0.01 : DURATION.cardHover, ease: EASE_PREMIUM };
  ...
  return (
    <div className="relative w-full min-h-[560px] lg:min-h-[680px] flex items-center overflow-hidden">
      <motion.img
        ...
        transition={{ duration: DURATION.heroSignature, ease: EASE_PREMIUM }}
      />
      <div className="absolute inset-0 bg-black/50" aria-hidden="true" />

      <motion.div
        className="relative z-10 mx-auto max-w-[1440px] w-full px-5 sm:px-8 lg:px-12 py-16 sm:py-20 lg:py-24"
        initial="hidden"
        animate="visible"
      ...
      >
        <div className="max-w-[640px] flex flex-col gap-7">
          ...
          <motion.h1
            variants={itemVariants}
            transition={itemTransition}
            className="font-heading font-bold text-[40px] sm:text-[54px] lg:text-[64px] text-white leading-[1.02] tracking-tight m-0"
          >
          ...
          <motion.p
            variants={itemVariants}
            transition={itemTransition}
            className="font-body text-[17px] sm:text-[18px] text-white/85 leading-[1.75] m-0 max-w-[56ch]"
          >
            {description}
          </motion.p>
          <motion.div variants={itemVariants} transition={itemTransition} className="flex flex-wrap items-center gap-x-5 gap-y-2 mt-1">
          ...
          <motion.div variants={itemVariants} transition={itemTransition} className="flex flex-wrap items-center gap-x-6 gap-y-3 mt-3">
            <button
              onClick={onRequestQuote}
              className="self-start inline-flex items-center justify-center gap-2.5 min-h-[44px] bg-brand-600 hover:bg-[#a80000] text-white font-heading font-bold text-[13.5px] uppercase tracking-[0.05em] leading-none px-8 py-[18px] rounded-[200px] transition-all duration-200 hover:-translate-y-0.5 shadow-[0_8px_24px_rgba(228,10,24,0.35)] hover:shadow-[0_10px_30px_rgba(228,10,24,0.45)] cursor-pointer focus:outline-none focus-gold"
            ...
            <button
              ...
              className="font-body font-semibold text-[13.5px] text-white underline underline-offset-4 decoration-white/50 hover:decoration-white transition-colors duration-200 cursor-pointer bg-transparent border-none p-0 focus:outline-none focus-gold"
            >
              Request Pricing
            </button>
          </motion.div>
