      ...
      {visible && (
        <>
          ...
          <motion.div
            ...
            className="hidden md:block fixed top-0 inset-x-0 z-40 bg-white/97 backdrop-blur-md border-b border-surface-200/70 shadow-[0_4px_20px_rgba(36,30,24,0.06)]"
          >
            <div className="mx-auto max-w-[1440px] w-full px-6 lg:px-12 py-3.5 flex items-center justify-between gap-6">
              <span className="font-heading font-bold text-[16px] text-heading truncate">
                {productName}
              </span>
              <div className="flex items-center gap-3 flex-shrink-0">
                ...
                {tdsUrl && (
                  <a
                    ...
                    className="inline-flex items-center justify-center min-h-[40px] text-heading font-heading font-bold text-[12px] uppercase tracking-[0.05em] leading-none px-6 py-2.5 rounded-[200px] border border-surface-200 hover:border-brand-600/40 hover:text-brand-600 transition-all duration-300"
                  >
                    Download TDS
                  </a>
                )}
                <button
                  ...
                  className="inline-flex items-center justify-center min-h-[40px] font-body font-bold text-[12.5px] text-brand-600 hover:text-[#a80000] underline underline-offset-4 decoration-brand-600/40 cursor-pointer bg-transparent border-none px-2"
                >
                  Request Quote
                </button>
              </div>
          ...
          <motion.div
            ...
            exit={{ y: reduceMotion ? 0 : 60, opacity: 0 }}
            transition={reduceMotion ? { duration: 0.01 } : { type: 'spring', stiffness: 300, damping: 30 }}
            className="md:hidden fixed bottom-0 inset-x-0 z-40 bg-white/97 backdrop-blur-md border-t border-surface-200/70 shadow-[0_-4px_20px_rgba(36,30,24,0.08)]"
            style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}
          >
            <div className="px-3 py-2.5 flex flex-col gap-2">
              <span className="font-heading font-bold text-[13.5px] text-heading truncate px-1">
                {productName}
              </span>
              <div className="flex items-stretch gap-2">
                ...
                {tdsUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-1 inline-flex items-center justify-center bg-surface-50 hover:bg-surface-100 border border-surface-200 text-surface-800 font-heading font-bold text-[12px] uppercase tracking-[0.03em] leading-none min-h-[44px] rounded-[200px] transition-all duration-200"
                  >
                    TDS
                  </a>
                )}
                <button
                  onClick={onRequestQuote}
                  className="flex-1 inline-flex items-center justify-center bg-surface-50 hover:bg-surface-100 border border-surface-200 text-surface-800 font-heading font-bold text-[12px] uppercase tracking-[0.03em] leading-none min-h-[44px] rounded-[200px] transition-all duration-200 cursor-pointer"
                >
                  Quote
                </button>
              </div>
