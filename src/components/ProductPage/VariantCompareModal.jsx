          ...
          <div className="relative">
            ...
            >
              <table className="w-full border-collapse min-w-[560px]">
                <thead>
                  <tr>
                    <th className="sticky left-0 z-20 bg-white dark:bg-surface-900 text-left px-3 py-3 font-body text-[11px] font-bold uppercase tracking-wider text-surface-400 shadow-[2px_0_6px_-2px_rgba(20,16,12,0.08)]" />
                    {variants.map((v) => (
                ...
                <tbody>
                  {specColumns.map((key) => (
                    <tr key={key} className="border-t border-surface-200/70 dark:border-surface-800">
                      <th scope="row" className="sticky left-0 z-10 bg-white dark:bg-surface-900 text-left px-3 py-3.5 font-body text-[11.5px] font-semibold uppercase tracking-wider text-surface-400 dark:text-surface-500 whitespace-nowrap align-top shadow-[2px_0_6px_-2px_rgba(20,16,12,0.08)]">
                        {key}
                      </th>
                      {variants.map((v) => (
            ...
            <AnimatePresence>
              {!hasScrolled && (
                <motion.div
                  ...
                  className="lg:hidden pointer-events-none absolute top-0 right-0 bottom-0 w-20 bg-gradient-to-l from-white dark:from-surface-900 from-30% via-white/70 dark:via-surface-900/60 to-transparent flex items-center justify-end pr-3"
                  aria-hidden="true"
                >
                  <motion.svg
